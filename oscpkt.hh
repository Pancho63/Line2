...
class Message
{
    ...
    std::vector<std::pair<size_t, size_t> >
        arguments;   // array of pairs (pos,size), pos being an index into the 'storage' array.
...
public:
    ...
    public:
        ...
        bool isBool()
        {
            return currentTypeTag() == TYPE_TAG_TRUE || currentTypeTag() == TYPE_TAG_FALSE;
        }
  ...
  */
    ArgReader match(const std::string &test) const
    {
        return ArgReader(*this,
                         fullPatternMatch(address.c_str(), test.c_str()) ? OK_NO_ERROR
                                                                         : PATTERN_MISMATCH);
    }
    ...
    {
        return ArgReader(*this,
                         partialPatternMatch(address.c_str(), test.c_str()) ? OK_NO_ERROR
                                                                            : PATTERN_MISMATCH);
    }
    ...
    void buildFromRawData(const void *ptr, size_t sz)
    {
        ...
        const char *type_tags_end = (const char *) memchr(type_tags_beg,
                                                          0,
    ...
    void packMessage(Storage &s, bool write_size) const
    {
        ...
        if (write_size)
            pod2bytes<uint32_t>(uint32_t(ceil4(l_addr) + ceil4(l_type) + ceil4(storage.size())),
                                s.getBytes(4));
...
private:
    ...
    {
        os << "osc_address: '" << msg.address << "', types: '" << msg.type_tags
           << "', timetag=" << msg.time_tag << ", args=[";
    ...
    PacketWriter &endBundle()
    {
        if (bundles.size()) {
            if (storage.size() - bundles.back() == 16) {
                pod2bytes<uint32_t>(0,
                                    storage.getBytes(
                                        4)); // the 'empty bundle' case, not very elegant
            }
            if (bundles.size() > 1) { // no size stored for the top-level bundle
                pod2bytes<uint32_t>(uint32_t(storage.size() - bundles.back()),
...
private:
    std::vector<size_t>
        bundles; // hold the position in the storage array of the beginning marker of each bundle
    Storage storage;
    ErrorCode err;
};
